...
# PYTHONPATH (e.g., `python demos/needle_passing_demo.py` from a worktree root).
...
def build_needle_passing_reward() -> CompositeReward:
    ...
    Components:
    - DistanceReward: Dense cross-arm shaping for needle approach and handoff
    - OrientationReward: Alignment of needle trajectory with tissue plane
    - ActionPenalty: Smooth, energy-efficient motions across both arms
    - TimePenalty: Efficiency pressure on the cooperative handoff
    - SuccessReward: Sparse bonus on complete SURGEON-to-ASSISTANT handoff
    - CollisionPenalty: Avoid unintended tissue damage or arm-arm collision
...
def build_observation_config() -> ObservationConfig:
    ...
    metrics needed for the dual-arm cooperative handoff.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        "PPO agent to pass a needle between surgeon_arm and assistant_arm",
...
Examples:
  ...
  python demos/needle_passing_demo.py --headless --steps 10000
    ...
    print_banner(
        ...
        subtitle="Phase 32 — Demo Suite Polish (Dual-Arm)",
    )

    # Print the 5 task stages as narration lines following the 5-stage template.
    # Vocabulary rule: name scene bodies explicitly (surgeon_arm, assistant_arm,
    # curved_passing_needle, tissue_target). Cover the 4 sub-steps:
    # needle pick-up → target approach → pass-through → withdrawal.
    narration_lines = [
        format_narration_step(
            "Setup",
            "The agent operates the surgeon_arm and assistant_arm inside a "
            "needle-passing scene with one tissue_target and one "
            "curved_passing_needle.",
        ),
        format_narration_step(
            "Action",
            "The policy guides the surgeon_arm to pick up the "
            "curved_passing_needle, transport it above tissue_target, and "
            "drive the tip through.",
        ),
        format_narration_step(
            "Critical Moment",
            "The needle's exit trajectory must align with the assistant_arm "
            "end effector within 5mm; misalignment tears tissue_target. The "
            "policy plans 3 steps ahead.",
        ),
        format_narration_step(
            "Outcome",
            "The curved_passing_needle emerges from tissue_target; "
            "assistant_arm closes its gripper on the shaft; surgeon_arm "
            "releases. The handoff is complete.",
        ),
        format_narration_step(
            "Takeaway",
            "Needle-passing rewards dense cross-arm distance shaping plus a "
            "sparse success bonus, training in roughly 100k PPO timesteps "
            "on a single CPU.",
        ),
    ]
