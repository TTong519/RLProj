...
def _run_single_seed(
    ...
    try:
        ...
        training_config = TrainingConfig(
            ...
            simulator=backend,
            max_episode_steps=config.max_episode_steps if hasattr(config, "max_episode_steps") else 1000,
...
class ExperimentRunner:
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.base_output_dir = Path(config.output_dir or "results") / f"{self.experiment_name}_{self.timestamp}"
    ...
    def run(self) -> dict[tuple[str, str], dict[str, Any]]:
        ...
        for backend in backends:
            ...
            for algorithm in algorithms:
                ...
                # Prepare seed runs
                seed_runs = [(algorithm, backend, seed, self.config, algo_output_dir, self.config.hyperparameters)
                            for seed in seeds]
                ...
                if self.config.max_parallel > 1 and len(seeds) > 1:
                    with ProcessPoolExecutor(max_workers=self.config.max_parallel) as executor:
                        futures = [
                            executor.submit(_run_single_seed, *args)
                        ...
                        for future in as_completed(futures):
                            ...
                            if result.status == "success":
                                logger.info(f"    Seed {result.seed}: success (reward={result.eval_metrics.get('mean_reward', 'N/A'):.2f})")
                            else:
        ...
        # Handle DreamerV3 conditional stub
        if self.config.dreamer_comparison:
            for backend in backends:
                aggregated[("DreamerV3", backend)] = {"status": "pending — Phase 24"}
                ...
                if "status" in stats:
                    ...
                    json_results[key] = {
                        ...
                        "learning_curve_mean_std": {
                            "mean": stats["learning_curve_mean_std"]["mean"].to_dict() if hasattr(stats["learning_curve_mean_std"]["mean"], "to_dict") else str(stats["learning_curve_mean_std"]["mean"]),
                            "std": stats["learning_curve_mean_std"]["std"].to_dict() if hasattr(stats["learning_curve_mean_std"]["std"], "to_dict") else str(stats["learning_curve_mean_std"]["std"]),
    ...
    def _print_algo_summary(self, algorithm: str, backend: str, seeds: list[int], all_results: list) -> None:
        ...
        if RICH.available:
            ...
            for r in algo_results:
                if r.status == "success":
                    m = r.eval_metrics
                    print(f"    seed={r.seed}: reward={m.get('mean_reward', 0):.2f}, success={m.get('success_rate', 0):.1%}")
                else:
