...
- [x] 40-01-PLAN.md — Real `_build_agent` (PyPI 1.5.0 object API) + DMV3-09 regression guard + extended JAX-leak guard (Wave 1, CPU-runnable TDD RED→GREEN) [DMV3-07, DMV3-09]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 40-02-PLAN.md — Real `_train_loop`/`_evaluate`/`_save_checkpoint`/`_load_checkpoint` (manual Driver loop, D-07) + Phase 30 sentinel flip negative→positive with structural-only assertions (Wave 2, GPU-GREEN via CI) [DMV3-07, DMV3-09, DMV3-10]
...
| 40. Real DreamerV3 Integration + Sentinel Flip | 2/4 | In Progress|  |
