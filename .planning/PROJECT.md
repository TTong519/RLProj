...
**Next:** v0.7.0 (in progress — Phase 41 complete 2026-07-15) — GUI editor depth (render/sim-decoupled viewport, multi-view/lighting/gizmos/recording, editing UX, file/IO, perf/stability: GUI-11..15) + scene generation (more task templates, better LLM text→scene, VLM image→scene, procedural/batch gen, interactive LLM clarifying-question flow: GEN-01..05), plus a GUI bug-fix pass (dock-panel layout reset on rerun, <10fps, immobile scene preview). GUI first, scene gen second.
...
### Active (v0.7.0 — in progress; Phase 41 complete)

- [ ] **GUI editor depth** (GUI-11..15) — render/sim-decoupled viewport, multi-view/lighting/gizmos/recording, editing UX, file/IO, perf/stability
- [~] **GUI bug fixes** — ✓ dock-panel layout reset on rerun (GUI-18 / Phase 41, bug #3 fixed via `DockStateManager` + closeEvent teardown); raise frame rate above 10fps + animate the scene preview (GUI-11 / Phase 42, bugs #1/#2)
- [ ] **Scene generation** (GEN-01..05) — more task templates, better LLM text→scene, VLM image→scene, procedural/batch gen, interactive LLM clarifying-question flow
...
*Last updated: 2026-07-15 — v0.7.0 in progress (Phase 41 complete 2026-07-15: GUI-18 DockStateManager + closeEvent teardown, bug #3 fixed, code-review CR-01 fixed). GUI Editor Depth & Scene Generation. v0.6.0 Carried-Forward Debt Closure SHIPPED 2026-07-15: 6 phases (36–40.1) / 18 plans / 13-of-13 requirements closed; all phases VERIFIED `passed`. Test baseline 1,513 passing. See `.planning/milestones/v0.6.0-ROADMAP.md` for the archived milestone.*
