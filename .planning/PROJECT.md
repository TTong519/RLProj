...
**v0.6.0 progress:** Phases 36 (difficulty schema), 37 (scene-level `difficulty_blocks` + env wiring), 38 (3D fluid flag `dim_3d=True`), 39 (K8s PVC e2e + organ-mesh licensing ADR), 40 (real DreamerV3 integration + sentinel flip), and 40.1 (post-verification gap-close: DMV3-08 `checkpoint_dir` threading + Phase 38 advisories) complete. 6 phases / 18 plans done; 13/13 v0.6.0 requirements closed at the source level. Phase 40 VERIFIED (human_needed): source complete + CPU guards GREEN (regression guard, JAX-leak guard, `.ckpt` glob tests); 3 PRESENT_BEHAVIOR_UNVERIFIED truths (DMV3-07 real-agent runtime, DMV3-08 resume, DMV3-10 CI GREEN) deferred to the `dreamer-gpu` CI job per INV-8 by design — pending GitHub-hosted GPU Actions runner enablement on the repo account (USER OPS ACTION ITEM). **Phase 40.1 closed the DMV3-08 parent-side resume gap** — `checkpoint_dir` now threads into `_find_latest_checkpoint` + both call sites (CPU-runnable stub test verifies it) — and landed the Phase 38 advisories: CR-01 3D force-unit magnitude regression test (green) + real TWO_WAY obstacle-velocity feedback with a `dt/coupling_substeps` substep loop (2D byte-identical). Phase 40.1 VERIFIED — 14/14 must-haves, no gaps. Test baseline 1,513 passing (Phase 39's e2e test is CI-only; macOS local skips by design; Phase 40's GPU-gated tests SKIP cleanly on macOS per INV-8).
...
*Last updated: 2026-07-14 — Phase 40.1 complete (post-verification gap-close): 6 v0.6.0 phases / 18 plans done; 13/13 requirements closed at source level. Phase 40.1 closed the DMV3-08 parent-side resume-path gap (`checkpoint_dir` threaded into `_find_latest_checkpoint` + both call sites; CPU-runnable stub test verifies it) and landed the Phase 38 advisories (CR-01 3D force-unit magnitude test green + real TWO_WAY obstacle-velocity feedback + `dt/coupling_substeps` substep loop, 2D byte-identical). VERIFIED — 14/14 must-haves. Phase 40 remains VERIFIED (human_needed); GPU GREEN deferred to the `dreamer-gpu` CI job per INV-8, pending GitHub GPU Actions runner enablement (USER OPS ACTION ITEM before milestone close). See `.planning/phases/40.1-close-gap-dmv3-08-thread-checkpoint-dir-into-find-latest-che/40.1-VERIFICATION.md`.*
