...
**v0.6.0 progress:** Phases 36 (difficulty schema), 37 (scene-level `difficulty_blocks` + env wiring), 38 (3D fluid flag `dim_3d=True`), 39 (K8s PVC e2e + organ-mesh licensing ADR), and 40 (real DreamerV3 integration + sentinel flip) complete. All 5 phases / 16 plans done; 13/13 v0.6.0 requirements closed at the source level. Phase 40 VERIFIED (human_needed): source complete + CPU guards GREEN (regression guard, JAX-leak guard, `.ckpt` glob tests); 3 PRESENT_BEHAVIOR_UNVERIFIED truths (DMV3-07 real-agent runtime, DMV3-08 resume, DMV3-10 CI GREEN) deferred to the `dreamer-gpu` CI job per INV-8 by design — pending GitHub-hosted GPU Actions runner enablement on the repo account (USER OPS ACTION ITEM). Test baseline 1,513 passing (Phase 39's e2e test is CI-only; macOS local skips by design; Phase 40's GPU-gated tests SKIP cleanly on macOS per INV-8).
...
*Last updated: 2026-07-12 — Phase 40 verified (human_needed): all 5 v0.6.0 phases / 16 plans done; 13/13 requirements closed at source level. Real DreamerV3 integration + sentinel flip complete at source; CPU guards GREEN; GPU GREEN deferred to the `dreamer-gpu` CI job per INV-8, pending GitHub GPU Actions runner enablement (USER OPS ACTION ITEM before milestone close). See `.planning/phases/40-real-dreamerv3-integration-sentinel-flip/40-VERIFICATION.md`.*
